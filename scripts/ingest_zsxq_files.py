...
DEFERRED_PATH = ROOT / "data" / "zsxq_scanned_deferred.jsonl"  # 扫描件待 M5 OCR 清单
...
def _log_deferred(gid, topic_id, file_id, name) -> None:
    """记录一个扫描件到待 M5 OCR 清单（追加 jsonl）。"""
    try:
        with open(DEFERRED_PATH, "a", encoding="utf-8") as f:
            f.write(json.dumps({"group_id": gid, "topic_id": topic_id,
...
class DrainStop(Exception):
    """命中 1059（账号频率/滥用信号）→ 无人值守时立即停止整轮，今晚不再请求，明晚续。"""


# ── zsxq API：取下载地址（1059 立即停整轮）──────────────────────────────────────
def get_download_url(sess: requests.Session, file_id) -> str | None:
    """GET /v2/files/{file_id}/download_url，返回下载地址（防御性取字段）。

    1988（限流）由复用的 api_get 内部退避重试；1059（账号频率/滥用信号）→ 抛 DrainStop：
    无人值守默认「第一次 1059 就停到明晚」，绝不冷却后再戳（反复 1059 是封号路径）。
    """
    polite_sleep()
    ...
    try:
        ...
        if "1059" in str(e):
            raise DrainStop(
                "命中 zsxq code=1059（账号频率/滥用信号），立即停止本轮，"
                "今晚不再请求，明晚用 done 断点续传。") from e
        raise
...
def process_group(sess, gid: str, gname: str, done: set[str],
                  limit: int | None, dry_run: bool, skip_scanned: bool = True) -> dict:
    """处理单个星球：遍历帖子 → 挑 PDF 附件 → 下载入库。返回统计。

    skip_scanned=True（通宵默认）：无文字层的扫描件不 OCR，记 M5 待办并标 done。
    """
    global _printed_host
    stat = {"topics": 0, "pdf_found": 0, "skipped_nonpdf": 0,
            "skipped_done": 0, "ingested": 0, "failed": 0, "scanned_deferred": 0}
    ...
    for topic in iter_topics(sess, gid, limit):
        ...
        for file in topic_files(topic):
            ...
            # 取下载地址（DrainStop=1059 往上传，停整轮；其它异常仅跳过本文件）
            try:
            ...
            # 首个真实下载：打印 host，供主控判断限流来自哪一层
            if not _printed_host:
                host = urllib.parse.urlparse(dl_url).netloc
                print(f"[首个下载 host] {host}  ← 字节下载来自此host, 限流通常来自api.zsxq.com")
                _printed_host = True

            tmp_pdf = TMP_DIR / f"_zsxq_tmp_{file_id}.pdf"
            try:
                download_to(dl_url, tmp_pdf, auth_sess=sess)
                head = pdf_head_text(str(tmp_pdf))
                if skip_scanned and not head.strip():
                    # 扫描件(无文字层)：通宵不OCR(省CPU+避免拖垮VPN)，记M5待办, 标done不重抓
                    _log_deferred(gid, topic_id, file_id, name)
                    done.add(source_id); save_done(done)
                    stat["scanned_deferred"] += 1
                    print(f"[扫描件→M5待办] {source_id}  name={name!r}")
                    continue
                # 来源：从PDF内容识别出品机构(BofA/大摩/华创…)，识别不到退回渠道
                author = detect_firm(head) or (f"知识星球·{gname}" if gname else "知识星球")
                n = ingest_pdf(
                    str(tmp_pdf), title=title, author=author,
                    source_id=source_id, source_type="zsxq",
                    use_ocr=(not skip_scanned),  # 通宵只入文字型→不OCR; --ocr-scanned才开
                    skip_vectors=True,
                )
                if n >= 0:
                    done.add(source_id)
                    ...
                    stat["ingested"] += 1
                    print(f"[入库OK] {source_id}  chunks+={n}   来源={author}  name={name!r}")
            except DrainStop:
                raise
            except Exception as e:
                stat["failed"] += 1
...
def main() -> int:
    ...
    ap.add_argument("--ocr-scanned", action="store_true",
                    help="对扫描件也OCR(吃CPU)；默认关=通宵只入文字型PDF,扫描件记M5待办")
    ...
    skip_scanned = not args.ocr_scanned
    ...
    try:
        for gid in gids:
            ...
            print(f"\n===== 星球 {gid} {gname} =====")
            try:
                stat = process_group(sess, gid, gname, done, args.limit,
                                     args.dry_run, skip_scanned)
            except DrainStop as e:
                stopped = True
                print(f"\n🛑 {e}", file=sys.stderr)
                break
            for k in grand:
        ...
        if not args.dry_run:
            save_done(done)
    if stopped:
        print("⏸ 本轮因 1059 提前停止；已入库进度已落盘，明晚原命令续跑即可。")
